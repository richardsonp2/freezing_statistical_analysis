...
linechart_prepost <- ggplot(prepost_figure_ds, aes(x= prepost, y = mean, group = sex_stress, colour = sex_stress))+
 ...
 linechart_prepost <- linechart_prepost + scale_color_manual(labels = line_label ,values = orange_blue)
...
bar_chart_retval1 <- ggplot(recall_1_descr, aes(x = sex_stress, y = mean_extRecall, fill = interaction(Condition,sex_stress))) +
...
color_pall <- c("#078bd8","#01b7a2","#7fb38d", "#7fc467", "#a40000", "#284a29","#ff870f","#1c20fc")

bar_chart_retval1 <- bar_chart_retval1 + scale_fill_manual(values = color_pall)
